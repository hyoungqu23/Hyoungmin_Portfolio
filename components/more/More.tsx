...
const More = () => {
  return (
    ...
    >
      <h2 className="text-xl font-bold">법대생이 프론트엔드 개발자가 되기로 한 이유</h2>
      <motion.div
        initial={{ opacity: 0 }}
        whileInView={{ opacity: 1 }}
        transition={{ duration: 1.2 }}
      >
        첫 번째는 <span className="font-bold text-red-500">재미와 흥미</span>이다. 이는 업으로
        ...
        프로그래밍을 접했을 때, python으로 고작 'Hello, world!' 하나 찍는데 굉장히 재미있었다. 또한,
        애초에 새로운 분야를 학습할 수 있는 경험은 진귀하다고 생각하고, 새로운 도전은 늘 흥미롭다고
        생각하기 때문이다. 또한, 직접 만드는 것에 따라 결과물을 확인할 수 있다는 점이 성향에 맞고,
        내 성향과 적성에 맞는 프론트엔드를 시작하게 되었다.
      </motion.div>
      <motion.div
        initial={{ opacity: 0 }}
        whileInView={{ opacity: 1 }}
        transition={{ duration: 1.2 }}
      >
        두 번째로, <span className="font-bold text-red-500">시각화와 구현에 대한 욕구</span>가 그
